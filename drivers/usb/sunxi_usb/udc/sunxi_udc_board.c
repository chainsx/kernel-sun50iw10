...
u32  open_usb_clock(sunxi_udc_io_t *sunxi_udc_io)
{
	...
	|| defined(CONFIG_ARCH_SUN8IW18) || defined(CONFIG_ARCH_SUN50IW10)
	...
	if (sunxi_udc_io->ahb_otg
		...
		if (sunxi_udc_io->ahb_usb1)
			clk_prepare_enable(sunxi_udc_io->ahb_usb1);
		if (sunxi_udc_io->mod_usbphy1)
			clk_prepare_enable(sunxi_udc_io->mod_usbphy1);
#endif
...
#if defined(CONFIG_ARCH_SUN50IW10)
		if (sunxi_udc_io->mod_usbphy1)
			clk_disable_unprepare(sunxi_udc_io->mod_usbphy1);
		if (sunxi_udc_io->ahb_usb1)
			clk_disable_unprepare(sunxi_udc_io->ahb_usb1);
#endif
