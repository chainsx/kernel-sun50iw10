...
#if defined(CONFIG_ARCH_SUN50IW10)
	sunxi_udc_io->ahb_usb1 = of_clk_get(np, 2);
	if (IS_ERR(sunxi_udc_io->ahb_usb1)) {
		sunxi_udc_io->ahb_usb1 = NULL;
		DMSG_PANIC("ERR: get usb ahb_usb1 failed.\n");
		return -EINVAL;
	}
	sunxi_udc_io->mod_usbphy1 = of_clk_get(np, 3);
	if (IS_ERR(sunxi_udc_io->mod_usbphy1)) {
		sunxi_udc_io->mod_usbphy1 = NULL;
		DMSG_PANIC("ERR: get usb mod_usbphy1 failed.\n");
		return -EINVAL;
	}
#endif
	return 0;
...
static struct class_attribute udc_reg_class_attrs[] = {
	__ATTR(udc_reg, 0644, udc_reg_show, NULL),
	__ATTR_NULL,
};
...
static struct class udc_reg_class = {
	.name		= "udc_reg",
	.owner		= THIS_MODULE,
	.class_attrs	= udc_reg_class_attrs,
};
...
static int sunxi_udc_suspend(struct device *dev)
{
	...
	val = USBC_Readl(g_sunxi_udc_io.usb_vbase + USBPHYC_REG_o_PHYCTL);
	val |= (0x1 << USBC_PHY_CTL_SIDDQ);
	USBC_Writel(val, g_sunxi_udc_io.usb_vbase + USBPHYC_REG_o_PHYCTL);
