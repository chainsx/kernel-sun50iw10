...
static int fts_power_source_init(struct fts_ts_data *data)
{
    ...
    if (data->pdata->reg_vdd_name) {
		data->vdd = regulator_get(&data->client->dev, data->pdata->reg_vdd_name);
		if (IS_ERR(data->vdd))
		{
			rc = PTR_ERR(data->vdd);
    ...
    if (data->pdata->reg_avdd_name) {
		data->vcc_i2c = regulator_get(&data->client->dev, data->pdata->reg_avdd_name);
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    /* reset, irq gpio info */
    pdata->reset_gpio = of_get_named_gpio_flags(np, "focaltech,reset-gpio", 0, NULL);
    if (pdata->reset_gpio < 0)
    {
        FTS_ERROR("Unable to get reset_gpio");
    }
    pdata->irq_gpio_flags = 0;
    pdata->irq_gpio = of_get_named_gpio_flags(np, "focaltech,irq-gpio", 0, NULL);
...
#if FTS_POWER_SOURCE_CUST_EN
    rc = of_property_read_string(np, "focaltech,reg_vdd", &pdata->reg_vdd_name);
	if (rc) {
		FTS_ERROR("Unable to get focaltech,reg_vdd");
	}
    rc = of_property_read_string(np, "focaltech,reg_avdd", &pdata->reg_avdd_name);
	if (rc) {
		FTS_ERROR("Unable to get focaltech,reg_avdd");
	}
#endif
