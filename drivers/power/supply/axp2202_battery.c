...
static int __axp2202_reset_mcu(struct regmap *regmap)
{
	int ret = 0;
...
static int axp2202_reset_mcu(struct regmap *regmap)
{
	...
	ret = __axp2202_reset_mcu(regmap);
				...
				union power_supply_propval *val)
{
	...
	if (bat_power->is_suppended) {
		pr_info("bat is suspended. return now\n");
		return -EBUSY;
	}
