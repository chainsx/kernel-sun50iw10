...
struct extcon_dev *edev_g;
struct axp2202_usb_power *usb_power_g;
...
void axp2202_usb_device_extcon(void)
{
	extcon_set_state_sync(edev_g, EXTCON_USB, true);
	extcon_set_state_sync(edev_g, EXTCON_USB_HOST, false);
}
EXPORT_SYMBOL_GPL(axp2202_usb_device_extcon);

void axp2202_usb_host_extcon(void)
{
	extcon_set_state_sync(edev_g, EXTCON_USB, false);
	extcon_set_state_sync(edev_g, EXTCON_USB_HOST, true);
}
EXPORT_SYMBOL_GPL(axp2202_usb_host_extcon);

void axp2202_usb_null_extcon(void)
{
	extcon_set_state_sync(edev_g, EXTCON_USB, false);
	extcon_set_state_sync(edev_g, EXTCON_USB_HOST, false);
}
EXPORT_SYMBOL_GPL(axp2202_usb_null_extcon);

void axp2202_boosten(bool en)
{
	int ret = 0;
	struct regmap *regmap = usb_power_g->regmap;

	if (en) {
		ret = regmap_update_bits(regmap, AXP2202_MODULE_EN, BIT(4), BIT(4));
		usb_power_g->usb_host = true;
	} else {
		ret = regmap_update_bits(regmap, AXP2202_MODULE_EN, BIT(4), 0);
		regmap_update_bits(regmap, AXP2202_RBFET_CTRL, BIT(0), 0);
		usb_power_g->usb_host = false;
	}


	if (ret < 0)
		printk("AXP2202 set boost enable failed\n");
}
EXPORT_SYMBOL_GPL(axp2202_boosten);
...
static void axp2202_usb_set_headphone(struct work_struct *work)
{
	struct axp2202_usb_power *usb_power =
		 container_of(work, typeof(*usb_power), resume_work.work);
	uint32_t reg_val = 0;
...
static int axp2202_usb_probe(struct platform_device *pdev)
{
	...
	edev_g = usb_power->edev;
	usb_power_g = usb_power;
	...
	if (usb_power->dts_info.pmu_cc_logic_en) {
		INIT_DELAYED_WORK(&usb_power->resume_work, axp2202_usb_set_headphone);
	}

	/* fix the bug which can not detect type-c headphone insert
	 * when system reset or power on bt power-key.
	 */
	regmap_read(usb_power->regmap, AXP2202_CC_STAT0, &reg_val);
	if ((reg_val & 0xf) == 0x7)
		extcon_set_state_sync(usb_power->edev, EXTCON_JACK_HEADPHONE, true);
...
static int axp2202_usb_suspend(struct platform_device *p, pm_message_t state)
{
	...
	regmap_update_bits(regmap, AXP2202_MODULE_EN, BIT(4), 0);
...
static int axp2202_usb_resume(struct platform_device *p)
{
	...
	if (usb_power->usb_host == true) {
		regmap_update_bits(regmap, AXP2202_RBFET_CTRL, BIT(0), 1);
		regmap_update_bits(regmap, AXP2202_MODULE_EN, BIT(4), BIT(4));
	}

	if (dinfo->pmu_cc_logic_en) {
		regmap_update_bits(regmap, AXP2202_CLK_EN, BIT(3), BIT(3));
		schedule_delayed_work(&usb_power->resume_work, msecs_to_jiffies(500));
	}
