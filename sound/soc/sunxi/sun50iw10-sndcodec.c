...
#if defined(CONFIG_EXTCON) && defined(CONFIG_AXP2202_POWER)
...
#if defined(CONFIG_EXTCON) && defined(CONFIG_AXP2202_POWER)
static void sunxi_check_hp_detect_status(struct work_struct *work)
{
	struct sunxi_card_priv *priv =
		container_of(work, struct sunxi_card_priv, hp_detect_work.work);
	int jack_type = 0, reg_val = 0;
	struct sunxi_codec_info *sunxi_codec = snd_soc_codec_get_drvdata(priv->codec);
	struct codec_spk_config *spk_cfg = &(sunxi_codec->spk_config);

	pr_err("[%s] detect_state=%d, mute_state=%d\n", __func__,
			priv->detect_state, sunxi_codec->mute_state);
	mutex_lock(&priv->jack_mlock);
	if (priv->detect_state == PLUG_OUT) {

		sunxi_codec->hp_plugin = PLUG_OUT;
		/* Enable PA */
		snd_soc_update_bits(priv->codec, SUNXI_DAC_REG,
			(0x1 << LINEOUTLEN), (0x1 << LINEOUTLEN));
		snd_soc_update_bits(priv->codec, SUNXI_HEADPHONE_REG,
				(0x1 << HPPA_EN),
				(0x0 << HPPA_EN));

		if (gpio_is_valid(priv->usb_switch_sel))
			gpio_set_value(priv->usb_switch_sel, 0);
		if (spk_cfg->used && !sunxi_codec->mute_state)
			gpio_set_value(spk_cfg->spk_gpio, spk_cfg->pa_level);
	} else {
		sunxi_codec->hp_plugin = PLUG_IN;
		if (spk_cfg->used)
			gpio_set_value(spk_cfg->spk_gpio, !spk_cfg->pa_level);

		if (gpio_is_valid(priv->usb_switch_sel))
			gpio_set_value(priv->usb_switch_sel, 1);
		snd_soc_update_bits(priv->codec, SUNXI_DAC_REG,
				...
				void *ptr)
{
	...
	pr_debug("[%s] line:%d event=%d\n", __func__, __LINE__, event);
	if (event)
		priv->detect_state = PLUG_IN;
	else
		priv->detect_state = PLUG_OUT;
	schedule_delayed_work(&priv->hp_detect_work,
			msecs_to_jiffies(10));
...
static int sunxi_card_suspend(struct snd_soc_card *card)
{
	...
	struct sunxi_card_priv *priv = snd_soc_card_get_drvdata(card);
	if (gpio_is_valid(priv->usb_switch_sel))
		sunxi_gpio_iodisable(priv->usb_switch_sel);
#endif
...
static int sunxi_card_resume(struct snd_soc_card *card)
{
	...
	struct sunxi_card_priv *priv = snd_soc_card_get_drvdata(card);
	if (gpio_is_valid(priv->usb_switch_sel)) {
		gpio_direction_output(priv->usb_switch_sel, 1);
		gpio_set_value(priv->usb_switch_sel, 0);
	}
#endif
...
static int sunxi_card_dev_probe(struct platform_device *pdev)
{
	...
	if (of_property_read_bool(np, "extcon")) {
		...
		ret = of_get_named_gpio(np, "usb_switch_sel", 0);
		if (ret < 0) {
			priv->usb_switch_sel = 0;
			pr_err("%s: error invalid usb_switch_sel err: %d\n",
				__func__, ret);
		} else {
			priv->usb_switch_sel = ret;
	...
	INIT_DELAYED_WORK(&priv->hp_detect_work, sunxi_check_hp_detect_status);
	priv->detect_state = PLUG_OUT;
	 ...
	 *	EXTCON_USB,
	 *	EXTCON_USB_HOST,
	 *	EXTCON_JACK_HEADPHONE,  <------ index = 0
	 *	EXTCON_NONE,
	 *
	 * };
	 */
	if (priv->extdev && (priv->extdev->state & (1 << 2))) {
		priv->detect_state = PLUG_IN;
		schedule_delayed_work(&priv->hp_detect_work, msecs_to_jiffies(100));
	}
