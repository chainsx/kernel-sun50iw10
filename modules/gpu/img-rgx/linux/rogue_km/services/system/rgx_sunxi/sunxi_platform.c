...
/* get ic's p-sensor and cpu leakage current value */
static void sunxi_get_psensor_icpu(uint32_t *icpu, uint32_t *pcpu)
{
	sunxi_get_module_param_from_sid(icpu, SUN50IW10_ICPU_EFUSE_OFF, 4);
	*icpu &= 0xfff; /* the unit is 0.1mA */
	sunxi_get_module_param_from_sid(pcpu, SUN50IW10_PCPU_EFUSE_OFF, 4);
	*pcpu &= 0xfff;
}
...
static int sunxi_decide_pll(struct sunxi_platform *sunxi)
{
	...
	if (id_dts == 0x0800) {
		uint32_t bin_icpu, bin_pcpu;
		sunxi_get_psensor_icpu(&bin_icpu, &bin_pcpu);
		dev_info(sunxi->dev, "icpu is %d, pcpu is %d\n", bin_icpu, bin_pcpu);

		/* when chip_id is 0x0800, need to get ic's p-sensor and leakage
		 * current value. If the values meet the following conditions,the
		 * pll_gpu will be set 600MHz.
		 */
		if (bin_pcpu > 3050 && (bin_icpu >= 140 && bin_icpu < 240))
			true_core = 600 * 1000000;
	}
